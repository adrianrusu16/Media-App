...
- **Networking Boundary**: `BackendClient`/`RemoteRepository` and `AudioSourceClient`/`JamendoAudioSourceClient`/`RetryingAudioSourceClient` isolate transport, source-download, and retry concerns so core engine logic remains transport-agnostic. `JamendoAudioSourceClient` is tonic-first via a gRPC seam (`JamendoGrpcApi`) and keeps transport details outside engine-domain flows.
